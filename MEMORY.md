...
## Recent Wins
- [2026-03-27] ScaleStack webhook pipeline built end-to-end: `api/webhook.js` + Vercel serverless function handles Stripe `checkout.session.completed` → adds to Supabase allowlist → sends Resend welcome email. All 4 env vars configured in Vercel (STRIPE_WEBHOOK_SECRET, RESEND_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_KEY). Supabase allowlist table created. SSH push method restored (no token needed).
- [2026-03-27] ProfitPulse email login fixed — `verify_user()` now searches by email column instead of username. Login field label updated. Account creation still broken (needs debugging next session).
- [2026-03-27] FL Tax Shield fully integrated into ProfitPulse as `tax_shield.py` module (inlined calculator functions, loaded via importlib at startup). Standalone Streamlit app deprecated.
